...
* **화면 렌더링 (`Step2View`)**:
  ...
  - **밀서 UI**: 약속된 만남 시간(`meeting_time`), 만남 장소(`meeting_place`)가 해제되어 노출됩니다.
  - **힌트 UI 숨김 (최신 개편)**: 스포일러 방지 및 Step 3에서의 긴장감 높은 탐색을 유도하기 위해, 기존에 약속 편지에 직접 표시되던 상대방 힌트(`partner_hint`) UI 영역은 주석 처리되어 보이지 않게 감추어졌습니다.
...
* **화면 렌더링 (`Step3View`)**:
  - **착장 입력 폼 (최신 개편)**: Step 3 화면 상단에 "오늘의 착장 정보를 입력해 주세요 (예: 하얀색 셔츠)"라는 텍스트 인풋과 제출 버튼을 제공합니다. 사용자가 입력한 값은 파트너의 `partner_hint` 컬럼에 실시간으로 업데이트되어 상대방이 나를 식별하는 단서로 제공됩니다.
  - **쌍방 도착 동기화 (최신 개편)**: 내가 도착 버튼을 누르더라도 상대방이 아직 도착하지 않은 상태(`current_step` < 3)라면 상대방 프로필이 바로 열리지 않고 "상대방의 도착을 기다리고 있습니다..."라는 대기용 대기실 카드가 상시 표시됩니다. **나와 상대방 모두 도착 확인을 완료하였을 때(쌍방 상태가 충족되었을 때)** 비로소 블라인드 프로필 화면과 시그널 코드 입력 폼이 동시에 해제되어 오픈됩니다.
  - **블라인드 프로필 오픈**: 쌍방 도착 확인 시 열리는 프로필 정보는 상대방의 닉네임, 회사/소속, 직업 분류, 사는 곳(대표지역만 '서울', '창원' 등 정제 노출), 프로필 사진, MBTI, 착장 정보(`partner_hint`)를 보여주되, **개인 실명과 연락처는 마스킹(***) 처리**하여 안전성을 유지합니다.
  - **시그널 번호 4자리 인증**:
    - 두 참가자는 화면에 표시된 본인의 **4자리 시그널 코드**(`signal_code`)를 현장에서 대면 교환합니다.
    - 상대방의 코드를 인증 폼에 정확히 입력하여 일치하면 `[💎 보물찾기 완료]` 동작이 실행되어 `current_step`이 4로 전환됩니다.

#### [Step 4] 보물찾기 성공! 및 최종 선택 (Treasure Hunt & Double Opt-in)
* **화면 렌더링 (`Step4View`)**:
  - **축하 팝업 메시지 비활성화**: 4단계 진입 시의 기존 축하 팝업 모달은 주석 처리되어 실행되지 않습니다.
  - **티타임 안내 타이머**: 화면에 "티타임이 시작되었습니다. 2시간 뒤 최종 선택이 활성화됩니다" 텍스트를 고정 렌더링하되, 테스트 편의를 위해 최종 선택 버튼은 즉시 클릭이 가능하게 오픈해 둡니다.
  - **최종 선택 버튼 2개**:
    - `[예, 더 알아가고 싶어요]` (최종 수락)
    - `[아니오, 여기까지 할게요]` (최종 거절)
  - **거절 마찰 (Friction) 모달 가드**: `[아니오]` 버튼을 누르는 순간 바로 취소되지 않고, 모달 팝업으로 **"정말 거절하시겠습니까? 한 번 거절하면 되돌릴 수 없습니다."** 경고를 한 번 더 노출하여 무조건적인 거절 오작동을 방지하는 Yes/No 이중 방어막을 구축합니다.
  - **최종 연락처 공개 (Double Opt-in)**: 참가자 **양측이 모두 최종 선택에서 `[예]`(수락)를 눌렀을 때에만** 마스킹 처리되어 있던 상대방의 '실명'과 '연락처' 정보가 완전히 잠금 해제되어 화면에 투명하게 공개됩니다. 한쪽이라도 거절하거나 미선택 시에는 연락처 비공개 상태가 영구히 유지됩니다.
  - **프라이빗 다이닝 정보**: 호스트가 예약해 둔 다이닝 매장명, 코스 요리 정보, 상세 지상 주소 및 네이버 지도 아웃링크가 하단에 지속적으로 표출됩니다.
