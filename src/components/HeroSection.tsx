...
export default function HeroSection({ onOpenRegistration }: HeroSectionProps) {
  ...
  const faqData = [
    ...
    {
      q: "혼자 신청해도 어색하지 않을까요?",
      a: "참가자의 80% 이상이 혼자 신청하시는 분들입니다. 뻘쭘함은 걱정하지 마세요. 호스트가 세팅해 드리는 사일런트 뮤직과 와인이 10분 만에 어색함을 녹여드립니다. 동행의 눈치를 볼 필요 없이, 오롯이 밤바다의 낭만과 새로운 사람들과의 대화에 집중할 수 있어 오히려 혼자 오시는 것을 더욱 강력히 추천합니다."
    },
    {
      q: "처음 만나는 분들과의 밤 모임인데, 어떤 분들이 참석하나요? (참여자 스크리닝)",
      a: "포구트립은 누구나 입금만 한다고 참석할 수 있는 흔한 파티가 아닙니다. 사전 신청서를 통해 참가자의 연령대, 여행 스타일, 결을 1차적으로 스크리닝하여 핏(Fit)이 맞는 분들로만 소수 정예(8인) 그룹을 큐레이션 합니다. 또한, 행사 내내 전문 호스트가 상주하며 분위기와 안전을 철저하게 리드하므로 불쾌한 상황에 대한 걱정은 완전히 내려놓으셔도 좋습니다."
    }
  ];
